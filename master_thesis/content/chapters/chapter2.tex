...
\begin{figure}[H]       % TODO: check the notations used on this figure
    \ctikzfig{figures/state_of_the_art/special_cases/c_not_c_tilde}
    \caption{Example of graph $G$ that has a matching index of $1$ and a weighted matching index of $2$.        % TODO: replace weight attribution everywhere with weighting
        ...
        Another example of mixed-edge colouring and weighting that gives the same matching index on $G$ is available in figure \ref{fig:perfectly_mono}.   % TODO: check this information
...
It was first described by Chandran and Gajjala in~\cite{chandran}, and we are using the same concepts than them here.   % TODO: finish to update here.
...
\begin{proof}[Proof of Theorem \ref{thm:bound_edge_connectivity}]
    ...
    % TODO : find the proof again in the original paper
    ...
    If there were (non-monochromatic) perfect matchings containing edges of that colour, they are all destroyed, so there is no induced vertex colouring of $G$ anymore that has vertices of that removed colour.   % TODO: if we have time, all of this is a bit confusing (since we removed ``colour class'')
            ...
            \begin{enumerate}
                ...
                \item
                    ...
                    \begin{proof}[Proof of Claim \ref{claim:two-mixed-edges}]
                        ...
                        Because $G_\mu^w$ is perfectly monochromatic, the weights of $1_{V(G)}$ and $2_{V(G)}$ must be $1$.     % TODO: change this to kappa1 and kappa2
